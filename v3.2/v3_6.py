...
def _build_fractal_domain_pool_px(
    recon: np.ndarray,
    bs: int,
    n_out: int,
) -> Optional[np.ndarray]:
    """Build pixel domain pool with extended support. Returns (D, n_out, n_out)."""
    h, w = recon.shape
    dbs = bs * 2
    dny, dnx = h // dbs, w // dbs
    D_base = dny * dnx
    ...
    morton_order = _morton_order(dny, dnx)
    dy_all = (morton_order // dnx).astype(int)
    dx_all = (morton_order %  dnx).astype(int)

    pad = (n_out - bs) // 2
    ext_dbs = n_out * 2
    recon_padded = np.pad(recon, ((ext_dbs, ext_dbs), (ext_dbs, ext_dbs)), mode='reflect')

    base_px = np.empty((D_base, n_out, n_out), dtype=np.float64)
    for k in range(D_base):
        dy, dx = dy_all[k], dx_all[k]
        cy, cx = (dy * dbs + bs + ext_dbs), (dx * dbs + bs + ext_dbs)
        y0, y1 = cy - n_out, cy + n_out
        x0, x1 = cx - n_out, cx + n_out
        r = recon_padded[y0:y1, x0:x1]
        base_px[k] = (
...
def _fractal_reconstruct_affine_px(
    pool_px:    np.ndarray,             # (D, n_out, n_out)
    domain_idx: np.ndarray,             # (N,) int16
    alpha_q:    np.ndarray,             # (N,) int8
    beta_q:     Optional[np.ndarray],   # (N,) int8 or None
    bs:         int = 8,
) -> np.ndarray:
    """Reconstruct predicted pixel blocks from stored (domain_idx, alpha_q, beta_q)."""
    alpha = alpha_q.astype(np.float64) * _FRACTAL_ALPHA_MAX / _FRACTAL_ALPHA_HALF
    idx   = np.clip(domain_idx.astype(np.int64), 0, pool_px.shape[0] - 1)
    pred  = pool_px[idx] * alpha[:, None, None]
    if beta_q is not None:
        beta      = beta_q.astype(np.float64) * _FRACTAL_BETA_MAX / _FRACTAL_BETA_HALF

        pred += (beta / float(bs))[:, None, None]
    return pred
...
def _eboa_reconstruct_image(
    ...
    bs: int,
    pad: int = 1,
    fractal_blocks: Optional[np.ndarray] = None,
    fractal_weight: float = 0.0,
) -> np.ndarray:
    """Reconstruct image using Extended-Basis Overlap-Add (EBOA) with Fractal support.

    Evaluates DCT basis functions on an extended grid and blends them using a
    partition-of-unity window. Optionally blends in fractal-extrapolated content.
    """
    N = len(coeff_blocks)
    n_out = bs + 2 * pad
    ...
    # Blend in fractal content if available
    if fractal_blocks is not None and fractal_weight > 0:
        # fractal_blocks are expected to be (N, n_out, n_out)
        ext_blocks = (1.0 - fractal_weight) * ext_blocks + fractal_weight * fractal_blocks
...
class ChannelCodecResult:
    ...
    # Blending parameters (v3.5)
    eboa_pad: int = 1
    eboa_fractal_weight: float = 0.0
    ...
    def __getstate__(self) -> dict:
        ...
        for k in self._FRAC_NONE_KEYS:
            if k == "eboa_pad" and d.get(k) == 1: d.pop(k, None); continue
            if k == "eboa_fractal_weight" and d.get(k) == 0.0: d.pop(k, None); continue
    ...
    def __setstate__(self, state: dict) -> None:
        ...
        state.setdefault('eboa_pad', 1)
        state.setdefault('eboa_fractal_weight', 0.0)
...
class FractalHybridCodec:
    ...
    def _encode_channel(self, channel: np.ndarray, quality: int) -> ChannelCodecResult:
        ...
        # Blending Parameter Search (v3.5)
        best_recon1 = None
        ...
        # Pre-build pools if needed
        pool_result = _build_fractal_domain_pool(
            (idct2(recon_coeffs) + 128.0).reshape(ny, nx, bs, bs).transpose(0, 2, 1, 3).reshape(h, w),
            bs
        )

        # Candidate parameters
        pads = [1, 2] if bs >= 8 else [1]
        fweights = [0.0, 0.2, 0.4]

        for pad in pads:
            n_out = bs + 2 * pad
            # Build extended pixel pool
            pool_px = _build_fractal_domain_pool_px(
                (idct2(recon_coeffs) + 128.0).reshape(ny, nx, bs, bs).transpose(0, 2, 1, 3).reshape(h, w),
                bs, n_out
            )
            ...
            for fw in fweights:
                frac_blocks = None
                p1_params = None
                if fw > 0 and pool_result is not None:
                    # We need domain assignments. For speed, use v3.3-style assignment
                    p1_idx, p1_alpha, p1_beta, _, _ = _affine_predict(
                        ordered_coeffs, pool_result[0], pool_result[1], pool_result[2],
                        _make_nbhd_map(ny*nx, pool_result[2]), with_beta=True
                    )
                    p1_params = (p1_idx, p1_alpha, p1_beta)
                    frac_blocks = _fractal_reconstruct_affine_px(pool_px, p1_idx, p1_alpha, p1_beta, bs=bs)

                recon1_cand = _eboa_reconstruct_image(recon_coeffs, order, ny, nx, bs, pad=pad,
                                                    fractal_blocks=frac_blocks, fractal_weight=fw) + 128.0
                resid = padded - recon1_cand
                energy = np.sum(np.abs(resid))

                if energy < best_energy:
                    best_energy = energy
                    best_recon1 = recon1_cand
                    best_pad = pad
                    best_fweight = fw
                    best_p1_params = p1_params
        ...
        N_blocks = ny * nx
        pool_result_s25 = _build_fractal_domain_pool(recon1, bs)
        if pool_result_s25 is not None and N_blocks >= _FRACTAL_MIN_BLOCKS:
            pool_dct, pool_dc, D_base = pool_result_s25
            nbhd = _make_nbhd_map(N_blocks, D_base)

            # Re-use or re-calculate Pass 1 affine prediction
            if best_p1_params is not None:
                p1_idx, p1_alpha, p1_beta = best_p1_params
                # Recalculate residual against the ACTUAL recon1 (which might be different from search base)
                p1_alpha_r = p1_alpha.astype(np.float64) * _FRACTAL_ALPHA_MAX / _FRACTAL_ALPHA_HALF
                p1_beta_r = p1_beta.astype(np.float64) * _FRACTAL_BETA_MAX / _FRACTAL_BETA_HALF
                pred_dct_p1_raw = p1_alpha_r[:, None, None] * pool_dct[p1_idx.astype(np.int64)]
                pred_dct_p1_raw[:, 0, 0] += p1_beta_r
                p1_resid = ordered_coeffs - pred_dct_p1_raw
            else:
                p1_idx, p1_alpha, p1_beta, p1_resid, _ = _affine_predict(
            ...
            # Use FBOA for fractal prediction too
            n_out_s25 = bs + 2 * best_pad
            pool_px_recon = _build_fractal_domain_pool_px(recon1, bs, n_out_s25)
            frac_blocks_p1 = _fractal_reconstruct_affine_px(pool_px_recon, p1_idx, p1_alpha, p1_beta, bs=bs)
            recon_frac1 = _eboa_reconstruct_image(pred_dct_p1, order, ny, nx, bs, pad=best_pad, fractal_blocks=frac_blocks_p1, fractal_weight=best_fweight) + 128.0
            ...
            if haar_saving > haar_l1_pre * 0.20 or best_fweight > 0:
                ...
                p2_idx, p2_alpha, _, p2_resid, _ = _affine_predict(
                ...
                # Use FBOA for combined fractal prediction
                recon_frac12 = _eboa_reconstruct_image(pred_dct_p1 + pred_dct_p2, order, ny, nx, bs, pad=best_pad, fractal_blocks=frac_blocks_p1, fractal_weight=best_fweight) + 128.0
        ...
        for p in range(self.residual_passes):
            ...
            # With FBOA, the Stage-1 reconstruction is even smoother,
            # further reducing the high-frequency energy needed in the residuals.
            # We use a coarser step (1.75x) to achieve higher compression ratios.
            qstep = max(0.5, _quality_to_qbase(quality) * (0.85 ** p) * 1.75)
        ...
        return ChannelCodecResult(
            ...
            eboa_pad=best_pad,
            eboa_fractal_weight=best_fweight,
    ...
    def _decode_channel(self, data: ChannelCodecResult) -> np.ndarray:
        ...
        # Identical EBOA-based Stage-1 reconstruction (v3.5 parameters)
        recon_basic = (idct2(recon_c) + 128.0).reshape(ny, nx, bs, bs).transpose(0, 2, 1, 3).reshape(h, w)
        frac_blocks = None
        if data.eboa_fractal_weight > 0:
            pool_result = _build_fractal_domain_pool(recon_basic, bs)
            if pool_result:
                # We need the SAME p1_idx used during encoding.
                # In v3.3 it was calculated on unquantized coeffs.
                # Wait, if I calculate it on recon_c it will be different but consistent?
                # Actually, the assignments for Stage 2.5 are already stored!
                # But Stage 2.5 assignments are only stored if gain threshold was met.
                # This blending might need its own assignments or use Stage 2.5 ones.
                # Let's simplify: blending search during encoding uses Stage 2.5 assignments
                # IF they exist, or calculates them on the fly.
                # At decode, we need the indices to reconstruct the extrapolation.
                # If Stage 2.5 didn't engage, we don't have frac1_idx.
                # This suggests we should store the indices used for blending if different.
                # To keep bitstream small, let's only allow blending if Stage 2.5 fired.
                if data.frac1_idx is not None:
                    # Undo DPCM on frac1 indices
                    D_total = pool_result[0].shape[0]
                    def _dec_idx(arr, flag):
                        raw = _dpcm_decode(arr.astype(np.int16), flag)
                        return np.clip(raw.astype(np.int64), 0, D_total - 1).astype(np.int16)
                    def _dec_int8(arr, flag):
                        return _dpcm_decode(arr.astype(np.int16), flag).astype(np.int8)

                    idx1 = _dec_idx(data.frac1_idx, getattr(data, "frac1_idx_dpcm", False))
                    alp1 = _dec_int8(data.frac1_alpha, getattr(data, "frac1_alpha_dpcm", False))
                    bet1 = _dec_int8(data.frac1_beta, getattr(data, "frac1_beta_dpcm", False))

                    n_out = bs + 2 * data.eboa_pad
                    pool_px = _build_fractal_domain_pool_px(recon_basic, bs, n_out)
                    frac_blocks = _fractal_reconstruct_affine_px(pool_px, idx1, alp1, bet1, bs=bs)

        recon1 = _eboa_reconstruct_image(recon_c, order, ny, nx, bs, pad=data.eboa_pad,
                                        fractal_blocks=frac_blocks,
                                        fractal_weight=data.eboa_fractal_weight) + 128.0
        ...
        if getattr(data, "frac1_idx", None) is not None:
            ...
            if pool_result is not None:
                ...
                # Use FBOA for fractal reconstruction
                recon = _eboa_reconstruct_image(pred_dct, order, ny, nx, bs, pad=data.eboa_pad, fractal_blocks=frac_blocks, fractal_weight=data.eboa_fractal_weight) + 128.0
