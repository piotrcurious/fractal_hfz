...
class RestrictedUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        ...
        if "numpy" in module:
            if name in ("_reconstruct", "ndarray", "dtype", "_frombuffer"):
                import numpy as np
                # Map old locations to new ones for compatibility if needed,
                # but usually getattr on the correct numpy sub-module works.
                try:
                    m = __import__(module, fromlist=[name])
                    return getattr(m, name)
                except (ImportError, AttributeError):
                    return getattr(np, name)
        if name == "ChannelCodecResult":
...
def _apply_path_dct(coeffs: np.ndarray, quality: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # We use 6 positions with balanced steps to improve over simple DPCM
    # Further refinement for better compression
    path_qsteps = qbase * np.array([0.9, 1.1, 1.1, 1.3, 1.3, 1.3], dtype=np.float32)
    path_qcoeffs_list = []
...
class FractalHybridCodec:
    ...
    def _encode_channel(self, channel: np.ndarray, quality: int) -> ChannelCodecResult:
        ...
        # 1. Energy-steered Hamiltonian Traversal
        # We increase the weight of HF energy for steering to follow details better
        steering_grid = (hf_energy.reshape(ny, nx) ** 1.2) + (total_energy.reshape(ny, nx) * 0.1) + EPSILON
        order_xy = build_hilbert_fractal_order(state_grid, steering_grid)
    ...
    def decompress(self, blob: bytes) -> np.ndarray:
        ...
        self.residual_levels = int(payload.get("residual_levels", self.residual_levels))
        self.residual_passes = int(payload.get("residual_passes", self.residual_passes))
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Hybrid fractal-Z image compressor v3")
    ...
    if args.cmd == "decode":
        ...
        # Peek to get params if needed, but decompress method handles it
        # Actually, let's just use a default codec and let decompress do the work
